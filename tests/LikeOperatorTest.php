...
it('escapes a literal backslash', function () {
...
it('wraps an escaped term in a contains pattern', function () {
    expect(LikeOperator::containsPattern('50%'))->toBe('%50\\%%');
});

it('picks the operator for the driver', function () {
    $expected = driver() === 'pgsql' ? 'ILIKE' : 'LIKE';

    expect(LikeOperator::for(Document::query()))->toBe($expected);
})->group('integration');

it('finds rows containing a plain substring', function () {
...
it('treats a percent sign in the term as a literal', function () {
...
it('treats an underscore in the term as a literal', function () {
...
it('treats a backslash in the term as a literal', function () {
...
it('matches a substring of a date column', function () {
...
it('matches a substring of a timestamp column', function () {
